...
open Module Polynomial Algebra Subalgebra in
lemma RealExtension_is_RorC (K : Type) [Field K] [Algebra ℝ K] [FiniteDimensional ℝ K]:
  ...
  | 2 =>
    ...
    refine ⟨AlgEquiv.ofBijective φ ⟨RingHom.injective _, (AlgHom.range_eq_top _).1 ?_⟩⟩
...
instance (K A : Type u) [Field K] [Ring A] [Nontrivial A] [Algebra K A] (L : SubField K A) :
    Field L := inferInstanceAs (Field L.1)
...
lemma cor_two_1to2 (K A : Type u) [Field K] [Ring A] [Algebra K A] [FiniteDimensional K A]
    [Algebra.IsCentral K A] [IsSimpleRing A] (L : SubField K A) :
    Subalgebra.centralizer K L.1 = L.1 ↔ Module.finrank K A =
    Module.finrank K L * Module.finrank K L := sorry
...
lemma x2_is_real (x : Dˣ) (hx : ∀ (z : k), (x.1⁻¹) * (f k e z) * x = k.val z)
  ...
  have x2_is_central : x.1^2 ∈ Subalgebra.center ℝ D := by
      ...
      simp only [AlgHom.coe_comp, Subalgebra.coe_val, Function.comp_apply, Units.val_inv_eq_inv_val,
        ...
        IsUnit.mul_inv_cancel_right] at hx
      ...
      simp only [SetLike.coe_eq_coe, EmbeddingLike.apply_eq_iff_eq,
        Complex.mk.injEq, true_and] at x_commutes_k
...
lemma real_sq_in_R_or_V (x : D) : x^2 ∈ (algebraMap ℝ D).range →
    x ∈ (algebraMap ℝ D).range ∨ x ∈ V := by
  rintro ⟨r, hr⟩
  if h'' : x ∈ V then
    exact Or.inr h''
  else
    ...
    have eq1 : (x - algebraMap ℝ D (Real.sqrt r)) * ( x + algebraMap ℝ D (Real.sqrt r)) = 0 := by
      ...
      rw [← pow_two, ← hr, ← map_mul, show algebraMap ℝ D √r * x = x * algebraMap ℝ D √r from
...
lemma jij_eq_negi (x : Dˣ) (hx : ∀ (z : k), (x.1⁻¹) * (f k e z) * x = k.val z)
  ...
  rw [← hx, ← Complex.I, Complex.conj_I, ← mul_assoc, ← Subalgebra.coe_mul, mul_comm,
    Subalgebra.coe_mul, mul_assoc, mul_inv_cancel₀ (by
      ...
      simp_all only [f_apply, Subalgebra.coe_val, Subtype.forall, lt_self_iff_false]),
        mul_one, map_neg, Subalgebra.coe_neg]
...
abbrev toFun (x : Dˣ) (hx : ∀ (z : k), (x.1⁻¹) * (f k e z) * x = k.val z)
  ...
  i_mul_i := by simp [i_mul_i]
  j_mul_j := j_mul_j _ _ _ hx hDD
  i_mul_j := by rfl
  j_mul_i := by simp [j_mul_i_eq_neg_i_mul_j _ _ _ hx hDD, ← mul_assoc]; sorry
}
